...
function createMenuItem(obj, num, time){
  let newObj = {};
  newObj.name = obj;
  newObj.price = num;
  newObj.category = time;
  return newObj;
}
...
console.log(createMenuItem(obj1));
console.log(createMenuItem(obj2));
console.log(createMenuItem(obj3));
...
function getLongReviews(arr) {
  let newArr = [];
    let curString = '';
    for(let i = 0; i < arr.length; i++) {
      let curString = arr[i].feedback;
      curString.split(' ');
      if(curString.length >= 15) {
          newArr.push(arr[i].feedback);
      }
    }
    return newArr;
  }
